...
from reportlab.lib.pagesizes import letter, elevenSeventeen, A0  # 页面的标志尺寸(8.5*inch, 11*inch)
...
class Graphs:
    ...
    # 绘制标题
    @staticmethod
    def draw_instance_title(title: str):
        # 获取所有样式表
        style = getSampleStyleSheet()
        # 拿到标题样式
        ct = style['Normal']
    ...
    def draw_table(*args):
        ...
        table = Table(args, colWidths=None, style=style)
        return table
...
# 添加标题
content.append(Graphs.draw_title('数据库巡检报告'))
...
def set_instance_name(instance_name):
    content.append(Spacer(240, 50))
    content.append(Graphs.draw_instance_title('实例名：%s' %instance_name))


def pdf_data(check_project, data):
    global content
    # 添加小标题
    content.append(Graphs.draw_little_title(check_project))
    # 添加表格
    # data = [
    #     ('职位名称', '平均薪资', '较上年增长率'),
    #     ('数据分析师', '18.5K', '25%'),
...
def set_pdf():
    # 生成pdf文件
    global content
    doc = SimpleDocTemplate('report.pdf', pagesize=A0)
    doc.build(content)
